function [] = One2onePropagatedLabelSumTest(SPdata,SPparam,LORparam)
...
for i = range
    ...
    propagatedLabelSum = GenPropagatedLabelSum(unfoldedReducedPropagatedLabels,SPdata.Labels{1,1},testSize); % note: Labels{1,1} = Labels{labelType,Kndx}
    
    Lsp = MaxDataCost(propagatedLabelSum);
    L = ProjectLabelToImage(imSP,Lsp);
    
    testName = ['DwSumPropagatedLabels' '_retSetSize_' num2str(LORparam.retSetSize) '_kNN_' num2str(LORparam.kNN)];
